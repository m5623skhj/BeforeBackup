<?php
    ...
    if(IndexDB_Connect() === false)
    {
        ...
        LOG_System(0, 'update_account.php', "DB Connect Error, $connectError, ".__LINE__, 3);
        $ResultArray = array('result'=>'-50');
        echo json_encode($ResultArray);
        exit;
    }

    // 얻어온 정보를 사용하여 해당 유저가 어느 db 에 있는지와 accountno 를 얻기 위하여 쿼리를 보냄
    ...
    if($QueryResult === false)
    {
        ...
        if($err == 1064)
        {
            LOG_System(0, 'update_account.php', "QueryError : $Query, ".__LINE__);
        ...
        {
            LOG_System(0, 'update_account.php', 'Table Error : available, '.__LINE__);
        ...
        {
            LOG_System(0, 'update_account.php', "Error : $err, ".__LINE__);
            $ResultArray = array('result'=>'-1000');
            echo json_encode($ResultArray);
            exit;
        }  
    ...
    // {
    //     DB_Disconnect();
    //     LOG_System(0, 'update_account.php', "Account Is Null $ColmnKey = $UserData, ".__LINE__);
    //     $ResultArray = array('result'=>'-10');
    //     echo json_encode($ResultArray);
    //     exit;
    // }
    ...
    if($QueryResult === false)
    {
        ...
        {
            LOG_System(0, 'update_account.php', 'Table Error : dbconnect, '.__LINE__);
            $ResultArray = array('result'=>'-62');
            echo json_encode($ResultArray);
            exit;
        }
    ...
    // 해당 테이블에 accountno 와 email 정보가 있는지를 확인함
    $Query = 'SELECT `accountno`, `email` FROM account WHERE `accountno`='.$accountno;
    $QueryResult = DB_SendQuery($Query);
    if($QueryResult === false)
    {
        $err = mysqli_errno($g_DB_connect);
    ...
    if($row === NULL)
    {
        ...
        if($QueryResult === false)
        {
            ...
            if($err == 1062)
            {
                LOG_System(0, 'update_account.php', "Duplicate Insert Email '$Email', $DBNo, ".__LINE__, 3);
    ...
    // 갱신할 데이터에 대하여 쿼리를 보냄
    $Query = 'UPDATE account SET '.$ParameterString.' WHERE `accountno`='.$accountno;
    $QueryResult = DB_SendQuery($Query);
    if($QueryResult === false)
    {
        $err = mysqli_errno($g_DB_connect);
        ...
        {
            LOG_System(0, 'update_account.php', "Column Update Error $err, ".__LINE__);
