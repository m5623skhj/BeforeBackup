<?php
    ...
    // ---------------------------------------------------------------------
    // accountno 혹은 email 을 기준으로 회원 데이터를 얻어온다
    ...
    {
        LOG_System(0, 'select_contents', 'Json Decode Parameter Error', 3);
    ...
    if(IndexDB_Connect() === false)
    {
        ...
        LOG_System(0, 'select_contents', "shdb_info DB Connect Error, $connectError, ".__LINE__, 3);
        $ResultArray = array('result'=>'-50');
        echo json_encode($ResultArray);
        exit;
    }

    // dbno 와 accountno 전부를 가져와야되나?
    ...
    if($QueryResult === false)
    {
        ...
        if($err == 1064)
        {
            LOG_System(0, 'select_contents.php', "QueryError : $Query, ".__LINE__);
        ...
        {
            LOG_System(0, 'select_contents.php', 'Table Error : available, '.__LINE__);
        ...
        {
            LOG_System(0, 'select_contents.php', "Error : $err, ".__LINE__);
    ...
    if($row === NULL)
    {
        // 회원가입이 안돼있음
        DB_Disconnect();
        LOG_System(0, 'select_contents.php', "Account Is Null $ColmnKey = $UserData, ".__LINE__);
    ...
    {
        $connectError = mysqli_connect_errno();
        LOG_System(0, 'select_contents.php', "DB Connect Error, $connectError, ".__LINE__, 3);
        $ResultArray = array('result'=>'-50');
        echo json_encode($ResultArray);
        exit;
    }
    ...
    if($QueryResult === false)
    {
        ...
        {
            LOG_System(0, 'select_contents.php', 'Table Error : dbconnect, '.__LINE__);
            $ResultArray = array('result'=>'-62');
            echo json_encode($ResultArray);
            exit;
        }
    ...
    if($DBInfoRow == NULL)
    {
        LOG_System(0, 'select_contents.php', 'DBInfoRow Is NULL, '.__LINE__);
        $ResultArray = array('result'=>'-1000');
        echo json_encode($ResultArray);
        exit;
    }
    ...
    $Query = 'SELECT * FROM `contents` WHERE `accountno`='.$accountno;
    $QueryResult = DB_SendQuery($Query);
    if($QueryResult === false)
    {
        $err = mysqli_errno($g_DB_connect);
    ...
    if($row === NULL)
    {
        ...
        if($QueryResult === false)
        {
            ...
            if($err == 1062)
            {
                LOG_System(0, 'select_contents.php', "Duplicate Insert Email '$Email', $DBNo, ".__LINE__, 3);
